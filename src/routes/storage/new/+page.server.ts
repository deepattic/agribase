...
export const actions = {
	addStorage: async ({ request, locals }: {request: any, locals: any}) => {
		let formData = await request.formData();
		formData = removeEmptyFields(formData)
        formData.append('agent', locals.pb.authStore.record.id)

		if (!locals.pb.authStore.record) {
			throw error(400, { message: "You must be logged in to Add Storage" });
		}

		try {
            await locals.pb.collection('storage_facilities').create(formData)
		} catch (err) {
			console.error(err);
